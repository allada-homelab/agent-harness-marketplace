...
You are running `/get-shit-done-claude:run`. This is the **orchestrator** entry for a complex or
fan-out task. Your job is to plan and delegate — not to do all the work yourself. Read the
`get-shit-done` skill (from the `get-shit-done` module) for the full method (triage rubric, workflow
cookbook, verification); this command is the driver.

`$ARGUMENTS` is the task. If it is empty, ask the user what they want done and stop.

**Precondition:** GSD fans out through a workflow tool: the **Workflow** tool on Claude Code, or the
**`run_workflow`** tool on pi and dsh (provided by the harness's module-workflows bridge; on pi it
activates when this skill is invoked). If neither is available in this session, say so plainly and stop —
never silently no-op at the fan-out step.
...
4. **Fan out through the spine workflow.** The spine is this module's `workflows/gsd.workflow.js`.
   - **Claude Code:** invoke the **Workflow** tool with `scriptPath` set to that file inside the plugin
     root (Claude expands the plugin-root variable for you; if it is not expanded in your context, locate
     the file by globbing `**/get-shit-done-claude/workflows/gsd.workflow.js`).
   - **pi and dsh:** invoke **`run_workflow`** with `workflow: "get-shit-done-claude:gsd"`.
   - Either way, `args`: `{ "task": "<the full task>", "subtasks": [ …your triaged subtasks… ], "researchTopics": [ … ] }`.
   ...
   - **Point the user at progress.** Once the workflow is running, tell them where to watch: Claude's
     `/workflows` view, or on dsh the subagent catalog in the web UI (pi prints per-agent narration).
   - **`--dry-run`** in `$ARGUMENTS`: print your decomposition + per-subtask tiers + whether `--isolate` is
     ...
     directive in their message (e.g. `+500k`) — on Claude the spine stops before fan-out if under ~30k
     tokens remain (pi and dsh track no token budget, so that guard never fires there).
   - **Resume, don't restart (Claude).** If the invocation errors mid-run or returns partial results, do
     not rerun from scratch — fix the cause, then resume with
     `Workflow({ scriptPath, resumeFromRunId: "<runId>" })`; completed agents replay from cache. After a
     session loss, the `runId` is findable in `/workflows`. On pi and dsh there is no resume: fix the
     cause and re-invoke `run_workflow`, passing only the subtasks that did not complete.
...
loop-until-dry audit), see the `get-shit-done` module's `references/workflow-cookbook.md` and author a
custom workflow script instead.
