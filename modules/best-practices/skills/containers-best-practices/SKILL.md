...
- **DOCKER-026** — Never `curl ... | sh` without checksum verification — pin a release artifact and verify `sha256sum` before executing.
- **DOCKER-027** — Pin `# syntax=docker/dockerfile:1` at the top of every Dockerfile; without it, BuildKit features silently degrade.
- **DOCKER-028** — Use `COPY --exclude=<pattern>` (stable in `docker/dockerfile:1`) to filter unwanted files from multi-file copies instead of `COPY` + `RUN rm` or multi-COPY workarounds.
- **DOCKER-029** — Run `docker buildx build --check` as a Dockerfile lint step in CI; BuildKit's built-in linter (21 checks as of frontend 1.25) catches several rules automatically, including `CopyIgnoredFile`.
...
- **DOCKER-031** — Use `COPY --parents` to preserve source directory structure instead of flattening or post-copy `mkdir`.
...
- **COMPOSE-001** — Use `depends_on` with `condition: service_healthy`, not bare `depends_on`.
...
- **COMPOSE-009** — Use `restart: unless-stopped` for long-lived services (not `always`).
...
- **COMPOSE-012** — One-shot init/migration containers use `restart: "no"` and downstream services depend on them with `condition: service_completed_successfully`.
- **COMPOSE-013** — For Postgres, put `PGDATA` in a subdirectory of the mount (`/var/lib/postgresql/data/pgdata`) to avoid initdb permission issues on first boot.
...
- **COMPOSE-015** — Use `develop.watch` (Compose ≥ 2.22) for hot-reload — selective sync/rebuild per path beats bare bind-mounting the workspace.
...
- **COMPOSE-017** — Set `pull_policy` explicitly on shared / production stacks; the default `missing` silently runs stale images when registries get new pushes.
...
- **COMPOSE-019** — Use the `include:` directive (Compose ≥2.20) to compose modular sub-applications into a parent project; distinct from `extends:` (single-service) and `-f` (file-level merge).
...
- **COMPOSE-021** — Use the top-level `models:` block (Compose ≥2.35) for declarative AI-model dependencies via Docker Model Runner; connection URL injected as env var.
...
- **COMPOSE-026** — Declare `logging:` with `max-size` + `max-file` on long-lived services; default `json-file` has no size limit and silently fills the host disk.
...
- **COMPOSE-028** — Require Compose ≥2.40.2 (or v5) before using `include:` with OCI/registry sources — CVE-2025-62725 path-traversal via artifact layer annotations.
...
- **BUILDX-005** — The default `docker` driver has limited cache export and **no multi-arch**. Switch to `docker-container` (or `kubernetes`) for full BuildKit features. `setup-buildx-action` does this for you in CI.
- **BUILDX-006** — For non-trivial multi-arch builds, use native runners per architecture (multi-node builder, or separate CI jobs + `imagetools create`). QEMU emulation is 3–10× slower for CPU-bound builds.
- **BUILDX-007** — Use `docker buildx debug build` (GA since buildx v0.33.0) — or `docker buildx dap build` for editor integration — for interactive step-through of failing builds; replaces the `RUN sleep 999` + `docker exec` hack.
- **BUILDX-008** — `oci-mediatypes=true,image-manifest=true` is the default since BuildKit v0.21, so `--sbom=true --provenance=mode=max` round-trips on modern registries; only legacy registries still need the explicit flag or a `mode=min` fallback.
...
- **SEC-002** — Use BuildKit (`DOCKER_BUILDKIT=1` or `docker buildx`). Many modern features depend on it.
...
- **SEC-006** — Use BuildKit secret mounts for build-time credentials; never `ARG` them.
- **SEC-008** — Minimize `RUN` layers without sacrificing readability; combine related steps, not unrelated ones.
- **SEC-009** — Use Compose's `secrets:` block (mounted as files under `/run/secrets/`) instead of `environment:` for any sensitive value — env vars leak via `docker inspect`, `/proc/<pid>/environ`, and logs.
- **SEC-010** — Image references with `@sha256:` digest must be fully literal; don't templatize the digest in Jinja/copier (template drift makes the version+digest pair inconsistent) and don't split version + digest across separate variables (Renovate/Dependabot can only keep them in lockstep when they're a literal pair on one line).
...
- **SEC-017** — Prefer `COPY` over `ADD`; avoid `ADD <url>` (no checksum, silent supply chain) and `ADD foo.tar.gz` (invisible auto-extract).
...
- **SEC-020** — Use `RUN --mount=type=ssh` + `docker buildx build --ssh default` for SSH-authenticated git/dep access during builds; never `COPY id_rsa` or `ARG SSH_PRIVATE_KEY`.
- **SEC-021** — Pin third-party scanner/tool images by digest and CI Actions by commit SHA — floating tags get hijacked (Trivy supply-chain compromise, 2026).
...
- **SEC-023** — Prefer a hardened/minimal verified base (Docker Hardened Images, Chainguard, distroless) for production runtimes.
...
- **UV-001** — Install `uv` via `COPY --from=ghcr.io/astral-sh/uv:VERSION@sha256:DIGEST /uv /uvx /bin/`. Pin the literal version+digest pair (SEC-010); the uv image is distroless, so the copy carries no extra weight.
- **UV-002** — Two-pass sync: bind-mount manifests + `uv sync --locked --no-install-project` first; then copy source + `uv sync --locked`. Dependency layer caches independently from source edits.
...
- **UV-009** — Point `UV_TOOL_BIN_DIR` at the system bin dir for derived images that install CLI tools via `uv tool install`, so the binaries land on `PATH` instead of the per-user uv-tools bin dir.
