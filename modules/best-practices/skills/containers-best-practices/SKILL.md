...
- **UV-009** — Point `UV_TOOL_BIN_DIR` at the system bin dir for derived images that install CLI tools via `uv tool install`, so the binaries land on `PATH` instead of uv's default executable dir (`~/.local/bin`).
