...
Referenced by the `/dual-agent-pr-review` command
(`~/.claude/commands/dual-agent-pr-review.md`). Not a skill: nothing auto-loads this,
so the command must be invoked explicitly.
...
```bash
~/.agents/lib/dual-agent-pr-review/scripts/dual_review.sh preflight
```
...
```bash
~/.agents/lib/dual-agent-pr-review/scripts/dual_review.sh prepare \
...
```bash
~/.agents/lib/dual-agent-pr-review/scripts/dual_review.sh run <RUN_DIR> round1 findings
```
...
```bash
~/.agents/lib/dual-agent-pr-review/scripts/dual_review.sh status <RUN_DIR> round1
```
...
Compose each side's prompt from `~/.agents/lib/dual-agent-pr-review/references/comment-brief.md`, filling
...
```bash
~/.agents/lib/dual-agent-pr-review/scripts/dual_review.sh run <RUN_DIR> comments_round comment-verdicts
```
...
`~/.agents/lib/dual-agent-pr-review/references/reconciliation-brief.md`, filling every `{{PLACEHOLDER}}`, and write
them to `<RUN_DIR>/roundN/{claude,codex}.prompt.md`. Then:

```bash
~/.agents/lib/dual-agent-pr-review/scripts/dual_review.sh run <RUN_DIR> round2 verdicts
```
